  ...
  const PEER = 'NewsletterMailingPeer';
	...
	public function setDateSent($v)
	{
		...
		if ( $this->date_sent !== null || $dt !== null ) {
			...
			$currNorm = ($this->date_sent !== null && $tmpDt = new DateTime($this->date_sent)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->date_sent = ($dt ? $dt->format('Y-m-d H:i:s') : null);
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 9; // 9 = NewsletterMailingPeer::NUM_COLUMNS - NewsletterMailingPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(NewsletterMailingPeer::ID) ) {
					...
					$affectedRows += NewsletterMailingPeer::doUpdate($this, $con);
				}
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $includeForeignObjects = false)
	{
		...
		if ($includeForeignObjects) {
			if (null !== $this->aSubscriberGroup) {
				$result['SubscriberGroup'] = $this->aSubscriberGroup->toArray($keyType, $includeLazyLoadColumns, true);
			}
	...
	{
		$copyObj->setDateSent($this->date_sent);
	...
	public function getNewsletter(PropelPDO $con = null)
	{
		if ($this->aNewsletter === null && ($this->newsletter_id !== null)) {
			$this->aNewsletter = NewsletterQuery::create()->findPk($this->newsletter_id);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
	...
	public function getUser(PropelPDO $con = null)
	{
		if ($this->aUser === null && ($this->updated_by !== null)) {
			$this->aUser = UserQuery::create()->findPk($this->updated_by);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
