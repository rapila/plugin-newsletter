...
abstract class BaseNewsletterQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'rapila', $modelName = 'Newsletter', $modelAlias = null)
	{
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new Newsletter();
			$obj->hydrate($row);
			NewsletterPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
	...
	public function filterByNewsletterMailing($newsletterMailing, $comparison = null)
	{
		...
		} elseif ($newsletterMailing instanceof PropelCollection) {
			return $this
