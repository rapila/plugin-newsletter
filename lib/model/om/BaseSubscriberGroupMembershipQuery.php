...
abstract class BaseSubscriberGroupMembershipQuery extends ModelCriteria
{
     ...
     * @param array $key Primary key to use for the query
                         A Primary key composition: [$subscriber_id, $subscriber_group_id]
     * @param     PropelPDO $con an optional connection object
     *
     * @return   SubscriberGroupMembership|SubscriberGroupMembership[]|mixed the result, formatted by the current formatter
     */
    ...
    protected function findPkSimple($key, $con)
    {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            SubscriberGroupMembershipPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
        }
        $stmt->closeCursor();
     ...
     * @param     PropelPDO $con an optional connection object
     *
     * @return PropelObjectCollection|SubscriberGroupMembership[]|mixed the list of results, formatted by the current formatter
     */
