...
abstract class BaseNewsletterMailingQuery extends ModelCriteria
{
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			NewsletterMailingPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
	 ...
	 *
	 * @param     Newsletter|PropelCollection $newsletter The related object(s) to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    NewsletterMailingQuery The current query, for fluid interface
	 */
	public function filterByNewsletter($newsletter, $comparison = null)
	{
		...
		} elseif ($newsletter instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this
